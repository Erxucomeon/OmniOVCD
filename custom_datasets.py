...
class CDDataset(BaseSegDataset):
    ...
    def load_data_list(self):
        ...
        支持两张图像（img1_path 和 img2_path）
        """
        data_list = []
        img1_dir = self.data_prefix.get('img1_path', None)
        img2_dir = self.data_prefix.get('img2_path', None)  # 第二张图像目录
...
class CHN6_CUGDataset(BaseSegDataset):
    ...
    def load_data_list(self):
        """Load annotation from directory or annotation file.
        Override to handle filename mapping: _sat.jpg -> _mask.png
        ...
        if not osp.isdir(self.ann_file) and self.ann_file:
            ...
            for line in lines:
                ...
                if ann_dir is not None:
                    # Replace _sat with _mask for segmentation map filename
                    seg_map_name = img_name.replace('_sat', '_mask')
                    seg_map = seg_map_name + self.seg_map_suffix
                    data_info['seg_map_path'] = osp.join(ann_dir, seg_map)
                data_info['label_map'] = self.label_map
                data_info['reduce_zero_label'] = self.reduce_zero_label
                data_info['seg_fields'] = []
            ...
            for img in fileio.list_dir_or_file(
                ...
                if ann_dir is not None:
                    # Remove image suffix and replace _sat with _mask
                    img_basename = img[:-_suffix_len]
                    seg_map_name = img_basename.replace('_sat', '_mask')
                    seg_map = seg_map_name + self.seg_map_suffix
                    data_info['seg_map_path'] = osp.join(ann_dir, seg_map)
                data_info['label_map'] = self.label_map
                data_info['reduce_zero_label'] = self.reduce_zero_label
